...
tot_num_param = 9 #total number of parameters
...
@parameters B104 B106 B108 B109 B204 B205 l B107 B206

eqs = [D(x) ~ B104*x*x+B106*x*x*x+B108*x*y+B109*x*x*y+l+B107*y*y*y #note Andrew: k2 is the real parameter on the x,y term. I further simplified. 

D(y) ~ B204*x*x+B205*y*y+B206*x*x*x]

@mtkbuild ags_sys = ODESystem(eqs,t)

p_opt = [9.785506000000000, -9.999987000000001, -9.407997000000000, 1.987823000000000, -2.915110000000000, 0.0019, -0.03500839,-3.242539000000000,-1.048649] 

ags_sys  = ODEProblem(ags_sys, [x => u0[1], y => u0[2]], tspan, [B104 => p_opt[1] B106 => p_opt[2] B108=> p_opt[3] B109=> 9.999995000000000 B204 => p_opt[4] B205 => p_opt[5] l=>p_opt[6] B107=>p_opt[8] B206=> p_opt[9]])


#build loss function (Andrew: we need to automate this using jinja2)
function model_loss(theta)
	#Andrew I'm not re-optimizing the initial conditions: could easily change
	_ags_sys = remake(ags_sys,u0=[x => u0[1], y => theta[7]], p=[B104 => theta[1] B106 => theta[2] B108=> theta[3] B204 => theta[4] B205 => theta[5] l=>theta[6] B107=>theta[8] B206=>theta[9]])
...
scaled_plan = scaleLHC(plan,[(lb[1],ub[1]),(lb[2],ub[2]),(lb[3],ub[3]),(lb[4],ub[4]),(lb[5],ub[5]),(lb[6],ub[6]),(lb[7],ub[7]),(lb[8],ub[8]),(lb[9],ub[9])]) #could also define function for spreading seeds out differently
...
_ags_sys = remake(ags_sys,u0=[x => u0[1], y => r[7]], p=[B104 => r[1] B106 => r[2] B108=> r[3] B204 => r[4] B205 => r[5] l=>r[6] B107 => r[8] B206 => r[9]])
ags_sol = solve(_ags_sys, saveat=SaveTimes)
ags_sol_rs = reduce(hcat, ags_sol.u) #reshape
CSV.write("8AC_fit.csv",DataFrame(ags_sol_rs,:auto),header=false) 
...
ylabel!("Tb")
display(p1) 
CSV.write("8AC_info.csv",DataFrame(store_p,:auto),header=false) 

hessian = ForwardDiff.hessian(z -> model_loss(z), r)
CSV.write("8AC_hessian.csv",DataFrame(hessian,:auto),header=false)
savefig(p1,"8AC")
...
ags_sol_rs = reduce(hcat, ags_sol.u) #reshape
CSV.write("8AC_val_fts.csv",DataFrame(ags_sol_rs,:auto),header=false) 
